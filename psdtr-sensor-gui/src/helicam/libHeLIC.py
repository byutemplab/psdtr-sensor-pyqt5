...
if __name__ == '__main__':
    logging.basicConfig(
        level=logging.DEBUG, format='%(asctime)s  %(levelname)s:%(module)s:%(lineno)d:%(funcName)s:%(message)s ')
...
class LibHeLIC(object):
    ...
    funcCBType = ct.CFUNCTYPE(
        ct.c_int, ct.c_long, ct.c_int, ct.c_int, ct.c_void_p)
...
# -------------------------------- GENERATED c2py.py ----------------------------
    ...
    def AllocCamData(self, idx, format, prop, extData, extDataSz):
        res = self.lib.HE_AllocCamData(
            self.handle, idx, format, prop, extData, extDataSz)
        return res
   ...
   # -------------------------------- GENERATED END --------------------------------
    ...
    class RegByName(object):
        ...
        def __getattr__(self, name):
            ...
            raise AttributeError(
                str(name)+' is not a valid reg attribute of '+str(self))
    ...
    class MapByName(object):
        ...
        def __getattr__(self, name):
            ...
            raise AttributeError(
                str(name)+' is not a valid map attribute of '+str(self))
        ...
        def as_dict(self):
            ...
            d = dict()
            for idx in range(rd.numMap):
                d[rd.maps[idx].id] = self.libHeLIC.GetMap(
                    ct.pointer(rd.maps[idx]))
            return d
    ...
    def Open(self, mode, *args, **kwargs):  # kwargs=keyword arguments
        ...
        if mode == 0:
            ...
            for k, v in items:
                ...
                s = s + k + '=' + v
            try:
                res = self.lib.HE_Open(
                    ct.byref(self.handle), mode, s.encode('utf-8'))
                print('res = ' + str(res))
    ...
    @staticmethod
    def GetSerials():
        # 127 is maximum number of devices
        nv = np.ndarray(127*17, dtype=np.uint8)
